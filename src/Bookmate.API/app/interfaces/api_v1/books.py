...
async def create_book(
    ...
    return await usecase.execute(payload, current_user.id.value)
...
async def list_books(
    ...
    return await usecase.execute(current_user.id.value)
...
async def list_available_books(
    ...
    stmt = select(BookORM).where(
        BookORM.owner_user_id == current_user.id.value,
        ~BookORM.id.in_(
...
async def get_book(
    ...
    repo = BookRepository(session)
    try:
        domain_book = await repo.get_by_id(book_id, current_user.id.value)
    except BookNotFound:
...
async def update_book(
    ...
    try:
        return await usecase.execute(book_id, payload, current_user.id.value)
    except BookNotFound:
        raise HTTPException(status_code=404, detail="Book not found")
...
async def delete_book(
    ...
    try:
        await usecase.execute(book_id, current_user.id.value)
    except BookNotFound:
        raise HTTPException(status_code=404, detail="Book not found")
...
async def create_books(
    payload: List[CreateBookRequest],
    session: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
):
    repo = BookRepository(session)
    ...
    results = []
    for book in payload:
        dto = await usecase.execute(book, current_user.id.value)
        results.append(dto)
